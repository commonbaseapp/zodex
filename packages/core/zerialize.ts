...
const zerializers = {
  ...
  ZodTuple: (def) => ({ type: "tuple", items: def.items.map(s) }),
  ZodSet: (def) => ({ type: "set", value: s(def.valueType) }),
  ZodArray: (def) => ({ type: "array", element: s(def.type) }),
...
export function zerialize(schema: ZodTypes): SzType {
